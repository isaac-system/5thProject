...
.compare{margin:auto;}
.comparetable{border: 1px solid #444444; border-collapse: collapse; width:100%}
...
<!-- 
	...
	<col width="31%">
		...
		<c:forEach var="glist" items="${comlist}">
		<td><a href="${path}/schList.do?gname=&neck=${glist.neck}">${glist.neck}</a></td>
		</c:forEach>
		</tr>
		...
		<c:forEach var="glist" items="${comlist}">
		<td><a href="${path}/schList.do?gname=&standard=${glist.standard}">${glist.standard}</a></td>
		</c:forEach>
		</tr>
