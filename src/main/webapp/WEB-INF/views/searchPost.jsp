...
<div id="container">
  ...
  <div style="display:flex;width:100%;flex-direction: row;justify-content: space-between;align-items: center;">
   ...
   <div>
	<h2>검색 결과</h2>
	<span id="searchCnt">검색 수 : ${latestPostList.size()}</span>
   <table class="table table-hover table-striped">
	<thead>
	...
	<tbody>
		<c:forEach var="latestPost" items="${latestPostList}">
		  	<tr>
		  		...
		  		<td><a href="/5th_project/post.do?id=${hotPost.id}&postId=${hotPost.id}&userId=${member.userid}">${latestPost.title} [${latestPost.reply_cnt}] 좋아요 ${latestPost.recommendation}</a></td>
