...
<div id="container">
  ...
  <div style="display:flex;width:100%;flex-direction: row;justify-content: space-between;align-items: center;">
  	<h2>커뮤니티</h2>
  	<form id="writeform" action="/5th_project/write.do">
  		<input type="hidden" name="boardId" value="${param.boardId}">
  		<input type="hidden" name="userId" value="${member.userid}">
  		<button id="writeBtn" type="button" style="border: 1px solid #000; background-color: white;" >글쓰기</button>
  	</form>
  </div>
	...
	<tbody>
		<c:forEach var="latestPost" items="${boardPostList}">
		  	<tr>
		  		...
		  		<td><a href="/5th_project/post.do?id=${latestPost.id}&postId=${latestPost.id}">
